...
if ! command -v git &> /dev/null; then
    ...
    sudo apt update && sudo apt install -y git || {
        ...
        echo -e "$TIME - Failed to install Git.\n" >> ~/Repository_Manager/logs/configuration.log
        exit 1
    }
    echo -e "${GREEN}Git successfully installed.${NC}" 
    echo -e "$TIME - Git successfully installed.\n" >> ~/Repository_Manager/logs/configuration.log
else
    echo -e "${GREEN}Git is already installed.${NC}"
    echo -e "$TIME - Git already installed.\n" >> ~/Repository_Manager/logs/configuration.log
fi
    ...
    while true; do
        ...
        case $SSH_KEY in
            [Yy]*)
                ...
                echo -e "$TIME - New SSH key successfully created.\n" >> ~/Repository_Manager/logs/configuration.log
                break ;;
            [Nn]*)
                echo -e "${GREEN}Keeping existing SSH key.${NC}"
                echo -e "$TIME - Keeping existing SSH key.\n" >> ~/Repository_Manager/logs/configuration.log
                break ;;
            *)
...
if [[ -f "$HOME/.gitconfig" ]]; then
    ...
    while true; do
        ...
        case $CHOICE in
            [Yy]*)
                ...
                echo -e "$TIME - File .gitconfig updated successfully.\n" >> ~/Repository_Manager/logs/configuration.log
                break ;;
            [Nn]*)
                echo -e "${GREEN}No changes were made to your .gitconfig file.${NC}"
                echo -e "$TIME - No changes were made to .gitconfig file.\n" >> ~/Repository_Manager/logs/configuration.log
                break ;;
            *)
                echo -e "${RED}Please answer y or n.${NC}" ;;
        esac
    ...
    echo -e "$TIME - File .gitconfig created successfully.\n" >> ~/Repository_Manager/logs/configuration.log
fi
    ...
    while true; do
        ...
        case $CHOICE in
            [Yy]*)
                ...
                echo -e "$TIME - File .bash_profile updated successfully.\n" >> ~/Repository_Manager/logs/configuration.log
                break ;;
            [Nn]*)
                echo -e "${GREEN}No changes were made to your .bash_profile file.${NC}"
                echo -e "$TIME - No changes were made to .bash_profile file.\n" >> ~/Repository_Manager/logs/configuration.log
                break ;;
            *)
                echo -e "${RED}Please answer y or n.${NC}" ;;
        esac
